...
def processCode(codeblocks, folder):
    ...
    for codeblock in render:
        ...
        if "Tegn Sirkel" in codeblock:
            ...
            if isInIfStatement == True:
                ...
                renderCode += f"\n\t\t\tDrawCircle({splits[0]}, {splits[1]}, {splits[2]}, GetColor(0x{splits[3][1:len(splits[3])]}ff));"
    ...
    with open("outline.c", "r") as file:
        ...
        projectCode = projectCode.replace("ClearBackground(bakgrunnFarge);", "ClearBackground(bakgrunnFarge);\t\t\t" + renderCode)
